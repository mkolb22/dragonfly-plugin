...
RULES_ANCHOR="$PROJECT_ROOT/data/anchors/rules.anchor.yaml"
if [ -f "$RULES_ANCHOR" ]; then
  echo ""
  echo "=== PROJECT RULES (from data/anchors/rules.anchor.yaml) ==="
  grep -A1 'text:' "$RULES_ANCHOR" | grep 'text:' | sed 's/.*text: "/- /' | sed 's/"$//' || true
  echo "=== Read data/anchors/rules.anchor.yaml after compaction for full context ==="
fi
