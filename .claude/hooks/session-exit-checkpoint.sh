#!/usr/bin/env bash
# Dragonfly Session Exit Checkpoint Hook
#
...
CREATED_AT=$(dragonfly_timestamp_iso)
CHECKPOINT_ID="chk-session-exit-${TIMESTAMP}"

# Get git info
GIT_BRANCH=$(dragonfly_git_branch)
LAST_COMMIT=$(dragonfly_git_last_commit)
UNCOMMITTED_FILES=$(dragonfly_git_uncommitted_count)
...
if dragonfly_state_available; then
    dragonfly_checkpoint_save "$CHECKPOINT_ID" "session-exit-${TIMESTAMP}" "session_exit" "$JSON_DATA"
fi
