...
|---|---|---|---|---|
| `name` | string | Yes | — | Concept name: `story`, `architecture`, `implementation`, `quality`, `version`, `security`, `documentation`, `verification`, `code-analysis`, `context`, `retrospective` |
...
**Memory module:** `dragonfly_plan_workflow` calls `memory_recall` internally to surface relevant architectural decisions and past workflow outcomes before generating recommendations. This is the primary feedback loop between past work and future planning.

**Evolve module:** Skills generated by `evolve_best` (with `save_as_skill: true`) are written to the skills directory read by `dragonfly_get_skills`. Evolved skills represent empirically improved concept prompts that replace or augment the default concept definitions.

**Analytics module:** `dragonfly_advance_workflow` writes provenance events to `stateDbPath`. The Analytics module's `dragonfly_timeline_view` reads these events to visualize workflow execution history.
