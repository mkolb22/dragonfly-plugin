...
2. **Framework status** — Call `dragonfly_framework_status` MCP tool
3. **Health status** — Call `dragonfly_health_get` MCP tool
4. **Workflow state** — Call `dragonfly_get_workflow_state` MCP tool (may return no active session)
...
Tests:     Run `npm test` in dragonfly-server for count
...
Git:       main @ 5958925 — feat: Wire sync rules into dragonfly_advance_workflow
...
Tests:     240 passing (dragonfly-server)
